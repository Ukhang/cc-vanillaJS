class PhysicsObject {
        ...
        class BlackHole extends PhysicsObject {
            ...
            draw(ctx) {
                ...
                ctx.beginPath();
                ctx.arc(this.position.x, this.position.y, this.eventHorizonRadius, 0, Math.PI * 2);
                ctx.fillStyle = 'black';
                ctx.fill();
                
                // Influence radius visualization (subtle glow)
                const gradient = ctx.createRadialGradient(
                    this.position.x, this.position.y, this.eventHorizonRadius,
                    this.position.x, this.position.y, this.influenceRadius
                );
                gradient.addColorStop(0, 'rgba(0, 100, 255, 0.3)');
                gradient.addColorStop(1, 'rgba(0, 100, 255, 0)');
                
                ctx.beginPath();
                ctx.arc(this.position.x, this.position.y, this.influenceRadius, 0, Math.PI * 2);
        ...
        class BlackHoleSimulation {
            ...
            setupControls() {
                ...
                accretionDiskSlider.addEventListener('input', (e) => {
                    ...
                    document.getElementById('accretionDiskValue').textContent = this.accretionDiskDensity;
                });
            ...
            initializeParticles() {
                ...
                for (let i = 0; i < this.particleCount; i++) {
                    ...
                    const distance = this.blackHole.influenceRadius * (0.3 + 0.7 * Math.random());
                    const angle = Math.random() * Math.PI * 2;
                    
                    // Add some height variation based on accretion disk density
                    const heightVariation = (1 - this.accretionDiskDensity) * 100;
                    const heightOffset = Math.random() * heightVariation - heightVariation / 2;
                    ...
                    const particle = new Particle(x, y, 0.1 + Math.random() * 0.5, generateColor());
                    
                    // Calculate orbital velocity for stable-ish orbit
                    // v = sqrt(G*M/r) perpendicular to radius
                    const orbitSpeed = Math.sqrt(0.1 * this.blackHole.mass / distance) * 0.3;
            ...
            update(deltaTime) {
                ...
                while (this.particles.length < this.particleCount) {
                    ...
                    const color = `hsl(${Math.random() * 60 + 180}, ${80 + Math.random() * 20}%, ${50 + Math.random() * 30}%)`;
