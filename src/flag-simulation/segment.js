class Segment {
  ...
  update() {
    ...
    const diff = subtract(this.particleA.loc, this.particleB.loc);
    const norm = normalize(diff);

    if (this.particleA.fixed) {
      this.particleB.loc = add(this.particleB.loc, scale(norm, delta));
    } else if (this.particleB.fixed) {
      this.particleA.loc = add(this.particleA.loc, scale(norm, -delta));
    } else {
      this.particleA.loc = add(this.particleA.loc, scale(norm, -delta / 2));
      this.particleB.loc = add(this.particleB.loc, scale(norm, delta / 2));
    }
  }
  draw(ctx) {
    ctx.beginPath();
